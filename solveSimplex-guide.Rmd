...
## Overview
**solveSimplex** is an R function that describes the equilibrium dynamics of _single-population_ evolutionary game theory games of **2-5 strategies**. All strategies are assumed to exist in _admixture_, and engage in _continuous time replicator dynamics_.
...
In the classic schoolyard game _Rock-Paper-Scissors_, players can choose from three strategies.
...
The competitive dynamics of _Rock-Paper-Scissors_ can be described using a **payoff matrix**. The payoff matrix has one row and one column for each strategy in the population. The **payoffs** are numerical values describing the fitness outcome to each strategy (by row) when in pairwise competition with each strategy (by column).
...
An n x n Jacobian matrix is calculated from the set of replicator equations $\dot{s}_{i}(t)$: 
...
Finally, the eigenvalues for this matrix are calculated and sorted by their real parts from largest to smallest (Sandholm 2010). 
...
- **Sink** equilibria are attractors - populations with any starting frequency of strategies will move toward the equilibrium frequency. The eigenvalues will all have negative real parts. 
- **Source** equilibria are repellers - if perturbed away from the equilibrium frequency, the population will not return to equilibrium. The eigenvalues will all have positive real parts. 
- **Saddle** equilibria act as attractors from some trajectories and repellers from others. The eigenvalues will have both positive and negative real parts. Here, saddles are classified by the number of positive eigenvalues present.
...
Because the payoffs for each strategy in **Rock-Paper-Scissors** are balanced, each strategy has equal fitness and the equilibrium is $\frac{1}{3}$ Rock, $\frac{1}{3}$ Paper, $\frac{1}{3}$ Scissors, and the equilibrium is a **sink**.
